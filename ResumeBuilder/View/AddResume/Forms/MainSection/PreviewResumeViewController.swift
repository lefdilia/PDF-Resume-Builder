...
//  ResumeBuilder
//
//  Created by Lefdili Alaoui Ayoub on 20/2/2022.
//
...
class PreviewResumeViewController : UIViewController {
    ...
    var initialObject: Initial? {
        didSet{
            ...
            templateColor = Theme.TemplatesOptions.Templates.Colors(rawValue: colorIndex) ?? templateColor
            
            let fontIndex = Int(initialObject.font)
            resumeFont = Fonts(rawValue: fontIndex) ?? resumeFont
            
            let imagetypeIndex = Int(initialObject.imageType)
            imageType = ImageType(rawValue: imagetypeIndex) ?? imageType
            
            let resumeSizeIndex = Int(initialObject.format)
            resumeSize = ResumeSize(rawValue: resumeSizeIndex) ?? resumeSize
            
            let templateIndex = initialObject.template
            selectedTemplate = Int(templateIndex)
            ...
            TemplatesGenerator(template: selectedTemplate,
                               ...
                               resumeData: resumeData).build { [weak self] template, _, _ in
    ...
    let topTextPresentation: UILabel = {
        let label = UILabel()
        let attributedText = NSMutableAttributedString(string: "Please choose a template from one of recomendation below.\n Add your informations, download, and get the job.", attributes: [.foregroundColor : UIColor.jumbo, .font: UIFont(name: "Avenir", size: 14) as Any])
    ...
    lazy var colorsStack: UIStackView = {
        ...
        for (index, color) in colors.enumerated() {
            ...
            _button.tag = index
            _button.addTarget(self, action: #selector(didChangeColor(_:)), for: .touchUpInside)
            
            listOfButtons.append(_button)
    ...
    func choosedTemplate(selected: Int, resumeSize: ResumeSize, imageType: ImageType, font: Fonts, data: ResumeData?) -> Templates {
        switch selected {
        case 0:
            return Templates.Cascade(size: resumeSize, imageType: imageType, font: font, colorOptions: templateColor, data: data)
        case 1:
            return Templates.Enfold(size: resumeSize, imageType: imageType, font: font, colorOptions: templateColor, data: data)
        default:
            return Templates.Cascade(size: resumeSize, imageType: imageType, font: font, colorOptions: templateColor, data: data)
        }
    }
    ...
    var templateColor: TemplatesColors = .HippiePink
    ...
    lazy var imagetypeOptionMenu: dropDownButton = {
        ...
        let button = dropDownButton(title: title, fontAlpha: 1, contentAlignment: .trailing)
    ...
    lazy var fontOptionMenu: dropDownButton = {
        ...
        let selectedValue = selectedItem["fontType"]
        let title = Fonts(rawValue: selectedValue ?? 3)?.title ?? "CenturyGothic"
    ...
    let formatLabel: UILabel = {
        let label = UILabel()
        label.attributedText = NSAttributedString(string: "Format", attributes: [.font : UIFont(name: Theme.nunitoSansSemiBold, size: 14) as Any])
        label.translatesAutoresizingMaskIntoConstraints = false
        return label
    }()
    ...
    override func viewDidLoad() {
        ...
        NSLayoutConstraint.activate([
            ...
            contentView.heightAnchor.constraint(equalTo: globalContentView.heightAnchor, multiplier: 0.63),
            
            //Switch Templates
            switchTemplatesButtonsStack.topAnchor.constraint(equalTo: contentView.bottomAnchor, constant: 20),
            ...
            formatStack.bottomAnchor.constraint(equalTo: globalContentView.bottomAnchor, constant: 0)
