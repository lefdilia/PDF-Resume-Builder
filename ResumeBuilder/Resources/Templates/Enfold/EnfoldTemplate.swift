...
class EnfoldTemplate: UIView {
    ...
    init(size: ResumeSize, imageType: ImageType, font: Fonts, colorOptions: TemplatesColors, data resumeData: ResumeData?) {
        ...
        let selectedColor = Theme.usedTemplate(with: .template(.Enfoldtpl, colorOptions))
        ...
        NSLayoutConstraint.activate([
            ...
            rightSide.leadingAnchor.constraint(equalTo: contentView.leadingAnchor),
            ...
            leftSide.leadingAnchor.constraint(equalTo: rightSide.trailingAnchor, constant: 0),
            leftSide.bottomAnchor.constraint(equalTo: contentView.bottomAnchor),
            leftSide.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: 0),
            righSideConstraint,
...
extension EnfoldTemplate {
    ...
    func createResume(filename: String? = nil, completion: (URL?, Error?)->()) -> Void {
        ...
        CreateResume.shared.generate(scrollView: scrollView, filename: filename) { resume, error in
    ...
    enum UsedFonts {
        ...
        var uiFont: UIFont? {
            ...
            case .profession(let _selectedFont):
                return UIFont(name: Theme.usedFont(with: .font(_selectedFont, .semiBold)), size: 13*0.93)
